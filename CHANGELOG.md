...
- Updated route helpers to match `duon/router`: use `any()` for methodless routes instead of `route()`, remove the passthrough `routes()` helper, and remove the `addGroup()` helper.
